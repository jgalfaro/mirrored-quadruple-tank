...
public class JKalman {
    ...
    /** temporary matrices Remote controler*/
    Matrix local_temp1;
    Matrix local_temp2;
    Matrix local_temp3;
    Matrix local_temp4;
...
*/      
        ...
        temp5 = new Matrix(mp, 1); 
         ////////////////////////////
        local_temp1 = new Matrix(dp, dp);
        local_temp2 = new Matrix(dp, mp);
        local_temp3 = new Matrix(dp, dp);
        local_temp4 = new Matrix(cp, dp);
        local_temp5 = new Matrix(dp, dp);        
    }
     ...
     */
    public Matrix Predict(Matrix control,boolean etat) {
    ...
    public Matrix Correct(final Matrix measurement) {
        ...
        // temp5 = z(k) - H*x'(k) 
        temp5 = null;//measurement_matrix.gemm(state_pre, measurement, -1, 1);
    ...
    */
    public Matrix Correct_d(final Matrix measurement) {
        ...
        temp3 = temp2.gemm(measurement_matrix.transpose(), measurement_noise_cov, 1, 1);
        Matrix temp4_var=error_cov_pre.times(measurement_matrix.transpose());
        Matrix ident = identity (2, 2);//new Matrix(1,1,1);	
        temp4=temp4_var.times(temp3.solve(ident));	
        // temp4 = inv(temp3)*temp2 = Kt(k) 
        //temp4 = temp3.solve(temp2);
	    // hokus pokus...
        ...
        // (2) Update estimate with measurement z(k)
        // temp5 = z(k) - H*x'(k) 
        temp5 = measurement_matrix.gemm(state_pre, measurement.transpose(), -1, 1);
    ...
    * Correct with the measurement, 
    */
     public Matrix Correct_residue(final Matrix residue) {
        ...
        // (2) Update estimate with measurement z(k)
        // temp5 = z(k) - H*x'(k) 
        temp5 = residue; //measurement_matrix.gemm(state_pre, measurement, -1, 1);
     ...
     * Remote control action
     */
    public Matrix Remote_Control_Action(Matrix feedback_gain){
        // Control action
        //////////////////////////
        //u=Lx(k)
        control_action= feedback_gain.times(state_post.minus(state_pre));
     ...
     * L: LQG control
     */
    public Matrix Local_LQG_control(Matrix ponderation1, Matrix ponderation2){
        
         // (1) Algebraic Riccati equation
         // A'*S  --> S=local_temp5
         local_temp3= (transition_matrix.transpose()).times(local_temp5);
         // B'*S  --> S=local_temp5
         local_temp4= (control_matrix.transpose()).times(local_temp5);
         // local_temp3*A + W
         Matrix inter= (local_temp3.times(transition_matrix)).plus(ponderation2);
         // (local_temp4*B + U)^-1 * local_temp4*A
         Matrix invers = (local_temp4.times(control_matrix)).plus(ponderation1).solve(local_temp4.times(transition_matrix));
         // S 
         local_temp5= inter.minus((local_temp3.times(control_matrix)).times(invers));
         
         // (2) feedback gain
         // (local_temp4*B + U)^-1 * local_temp4*A
         Matrix new_invers = (local_temp4.times(control_matrix)).plus(ponderation1).solve(local_temp4.times(transition_matrix));
         feedback_gain= new_invers.times(-1);
     ...
     * set residue
    */
    public void setResidue(Matrix residue) {
         temp5 = residue;  
    }
     ...
     * @param state_pre
     */
    public Matrix getDistanceEstimation() {
        return measurement_matrix.times(state_pre);
    }
}

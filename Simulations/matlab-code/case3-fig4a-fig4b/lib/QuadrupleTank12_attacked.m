classdef QuadrupleTank12_attacked < Process
	properties
	...
	methods
		function self = QuadrupleTank12_attacked(a, A, g, gamma, k)
			self.a = a;
			self.A = A;
			self.g = g;
			self.gamma = gamma;
		...
		function dxdt = derivatives(self, t, x, u, ~, ~)
            ...
            dxdt(10) = self.a(1)/self.A(5)*sqrt(2*self.g*xt(5)) + self.a(2)/self.A(5)*sqrt(2*self.g*xt(7)) - self.k(1)/self.A(5)*u(t,1) - self.k(2)/self.A(5)*u(t,2);

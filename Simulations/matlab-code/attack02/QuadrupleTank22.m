classdef QuadrupleTank22 < PCS.Process.Process
	properties
...
%             end
		...
		function dxdt = derivatives(self, t, x, u, ~, ~)
            ...
            dxdt(5) = self.a(1)/self.A(5)*sqrt(2*self.g*xt(1)) + self.a(2)/self.A(5)*sqrt(2*self.g*xt(2)) - self.k(1)/self.A(5)*u(t,1) - self.k(2)/self.A(5)*u(t,2); %- self.lambda(1)*self.w(1)*u(t,1) - self.lambda(2)*self.w(2)*u(t,2);

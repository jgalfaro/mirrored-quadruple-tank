classdef SPGA_attacked < PCS.Control.Controller
    properties
    ...
    methods
        function self = SPGA_attacked(K,xsym,hsym,gsym)
        ...
        function dxcdt = derivatives_attacked(~, ~, ~, ~, ~, ~, ~)
            dxcdt = [];
        end

        function u = outputs_attacked(self, t, ~, x, ~, ~, ~)
            xt = x(t);
            u=self.usym(xt);
        end

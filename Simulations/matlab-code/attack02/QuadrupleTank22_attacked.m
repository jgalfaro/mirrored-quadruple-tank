classdef QuadrupleTank22_attacked < PCS.Process.Process
	properties
	...
	methods
		function self = QuadrupleTank22_attacked(a, A, g, gamma, k, n, lambda, w)
...
%           end
		...
		function y_22_unattacked = outputs(self, t, x, u, ~, ~)
			%y = x(t, [1 2]);            
            
            y_22_unattacked = zeros(8,1);
  			y_22_unattacked(1) = x(t, 1);
            y_22_unattacked(2) = self.gamma(1)*self.k(1)/self.A(1) + self.n(1)*self.lambda(1)*self.w(1); % 1 3
            y_22_unattacked(3) = x(t, 2);
            y_22_unattacked(4) = self.gamma(2)*self.k(2)/self.A(2) + self.n(2)*self.lambda(2)*self.w(2); % 2 4
            y_22_unattacked(5) = x(t, 3);
            y_22_unattacked(6) = (1-self.gamma(2))*self.k(2)/self.A(3) + (1-self.n(2))*self.lambda(2)*self.w(2); % 2 4
            y_22_unattacked(7) = x(t, 4);
            y_22_unattacked(8) = (1-self.gamma(1))*self.k(1)/self.A(4) + (1-self.n(1))*self.lambda(2)*self.w(1); % 1 3

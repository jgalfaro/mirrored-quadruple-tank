classdef QuadrupleTank22_performance < Process
	properties
	...
	methods
		function self = QuadrupleTank22_performance(a, A, g, gamma, k, n, lambda, w)
			self.a = a;
			self.A = A;
			self.g = g;
			self.gamma = gamma;
		...
		function dxdt = derivatives(self, t, x, u, ~, ~)
            ...
            if t > 508
                ...
                if t > 550
                    ...
                    dxdt(5) = self.a(1)/self.A(5)*sqrt(2*self.g*xt(1)) + self.a(2)/self.A(5)*sqrt(2*self.g*xt(2)) - self.k(1)/self.A(5)*u(t,1) - self.k(2)/self.A(5)*u(t,2) - self.lambda(1)/self.A(5)*u(t,1) - self.lambda(2)/self.A(5)*u(t,2);
                end
           end

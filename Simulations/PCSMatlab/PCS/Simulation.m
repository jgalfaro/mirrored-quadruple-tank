classdef Simulation < handle
    ...
    methods
        function self = Simulation(controller,process)
            ...
            if nargin == 0
                self.sensor_links{1} = SensorLink();
                self.actuator_links{1} = ActuatorLink();
            else
                ...
                for i = 1:process.n_disturbances
                    self.d_sensors{i} = Sensor();
                end

                for i = 1:process.n_states
                    self.x_sensors{i} = Sensor();
                end

                for i = 1:process.n_outputs
                    self.y_sensors{i} = Sensor();
                end
    ...
    methods
        ...
        function set.controller(self,controller)
            if ~isscalar(controller) || ~isa(controller,'Controller')
                error ('Error: Controller must be a subclass of Controller.');
            end
        ...
        function set.process(self,process)
            if ~isscalar(process) || ~isa(process,'Process')
                error ('Error: Process must be a subclass of Process.');
            end
            ...
            self.sensor_links = [];
            self.add_sensor_link(SensorLink());
    ...
    methods
        ...
        function set_preloaded_disturbance(self,index,d,t,interp)
            ...
            elseif nargin == 4
                self.d_preloaded_fun{index} = self.create_signal_fun(t,d,InterpolationMethod.ZOH);
            else
        ...
        function set_preloaded_reference(self,index,r,t,interp)
            ...
            elseif nargin == 4
                self.r_preloaded_fun{index} = self.create_signal_fun(t,r,InterpolationMethod.ZOH);
            else
    ...
    methods (Hidden = true, Access = private)
        function [dxdt,d,r,u,y,xp_measured,y_measured,d_measured,xp_sent,y_sent,d_sent,u_sent,u_actuator] = derivatives(self,t,x)
            ...
            d_fun = self.create_signal_fun([self.t_sim t],[self.d_sim d],InterpolationMethod.ZOH);
            
            d_measured_fun = self.create_signal_fun([self.t_sim t],[self.d_measured_sim d_measured],InterpolationMethod.ZOH);
            ...
            r_fun = self.create_signal_fun([self.t_sim t],[self.r_sim r],InterpolationMethod.ZOH);
            ...
            xp_fun = self.create_signal_fun([self.t_sim t],[self.xp_sim xp],InterpolationMethod.Linear);
            
            xp_measured_fun = self.create_signal_fun([self.t_sim t],[self.xp_measured_sim xp_measured],InterpolationMethod.ZOH);
            
            xc = x(self.process.n_states+1:end);
            
            xc_fun = self.create_signal_fun([self.t_sim t],[self.xc_sim xc],InterpolationMethod.Linear);
            
            u_actuator_fun = self.create_signal_fun(self.t_sim,self.u_actuator_sim,InterpolationMethod.ZOH);
            ...
            y_measured_fun = self.create_signal_fun([self.t_sim t],[self.y_measured_sim y_measured],InterpolationMethod.ZOH);
            ...
            for i = 1:length(self.sensor_links)
                if self.sensor_links{i}.type == LinkType.ContinuousTime
            ...
            xp_sent_fun = self.create_signal_fun([self.t_sim t],[self.xp_sent_sim xp_sent],InterpolationMethod.ZOH);
            y_sent_fun = self.create_signal_fun([self.t_sim t],[self.y_sent_sim y_sent],InterpolationMethod.ZOH);
            d_sent_fun = self.create_signal_fun([self.t_sim t],[self.d_sent_sim d_sent],InterpolationMethod.ZOH);
            
            u = self.controller.outputs(t,xc_fun,xp_sent_fun,y_sent_fun,d_sent_fun,r_fun);
            
            u_fun = self.create_signal_fun([self.t_sim t],[self.u_sim u],InterpolationMethod.Linear);
            ...
            for i = 1:length(self.actuator_links)
                if self.actuator_links{i}.type == LinkType.ContinuousTime
            ...
            u_actuator_fun = self.create_signal_fun([self.t_sim t],[self.u_actuator_sim u_actuator],InterpolationMethod.ZOH);
        ...
        function [value,isterminal,direction] = events(self,t,x)
            ...
            x_measured_fun = self.create_signal_fun([self.t_sim t],[self.xp_measured_sim x_measured],InterpolationMethod.ZOH);
            y_measured_fun = self.create_signal_fun([self.t_sim t],[self.y_measured_sim y_measured],InterpolationMethod.ZOH);
            d_measured_fun = self.create_signal_fun([self.t_sim t],[self.d_measured_sim d_measured],InterpolationMethod.ZOH);
        ...
        function status = on_event(self,ie,te,accumulated_events)
            ...
            if n_ce > 0
                ...
                for i = 1:length(activated_ce_sensor_links)
                    ...
                    xp_measured_fun = self.create_signal_fun(self.t_sim,self.xp_measured_sim,InterpolationMethod.Linear);
                    y_measured_fun = self.create_signal_fun(self.t_sim,self.y_measured_sim,InterpolationMethod.Linear);
                    d_measured_fun = self.create_signal_fun(self.t_sim,self.d_measured_sim,InterpolationMethod.Linear);
                ...
                for i = 1:length(activated_ce_actuator_links)
                    ...
                    u_fun = self.create_signal_fun(self.t_sim,self.u_sim,InterpolationMethod.Linear);
            ...
            if n_de > 0
                ...
                for i = 1:length(activated_de_sensor_links)
                    ...
                    xp_measured_fun = self.create_signal_fun(self.t_sim,self.xp_measured_sim,InterpolationMethod.ZOH);
                    y_measured_fun = self.create_signal_fun(self.t_sim,self.y_measured_sim,InterpolationMethod.ZOH);
                    d_measured_fun = self.create_signal_fun(self.t_sim,self.d_measured_sim,InterpolationMethod.ZOH);
                    u_fun = self.create_signal_fun(self.t_sim,self.u_sim,InterpolationMethod.Linear);
        ...
        function data = end_simulation(self,time_vector)
            if isempty(time_vector)
                ...
                xp_function = self.create_signal_fun(self.t_sim,self.xp_sim,InterpolationMethod.Linear);
